...
// Tighter keyword phrases — exact specialist terms only
const KEYWORDS = [
...
// Job title MUST contain at least one of these terms to pass
...
// Job title containing any of these gets rejected outright
...
function isRelevantRole(title) {
  ...
  if (TITLE_BLOCKLIST.some(term => t.includes(term))) return false;

  // Accept only if allowlisted
  return TITLE_ALLOWLIST.some(term => t.includes(term));
}

function fetchJobs(country, keyword) {
  return new Promise((resolve) => {
    const query = encodeURIComponent(keyword);
    const url = `https://api.adzuna.com/v1/api/jobs/${country}/search/1?app_id=${APP_ID}&app_key=${APP_KEY}&what_phrase=${query}&results_per_page=20&sort_by=date&max_days_old=14&content-type=application/json`;
...
function buildHTML(jobs, generatedAt) {
  ...
  const noRoles = `<div class="no-roles">
    <p>No specialist roles found this week across 11 countries.</p>
    <p>This is normal — dedicated security awareness roles are rare. Check back next week.</p>
  </div>`;
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    header h1 { font-size: 1.75rem; font-weight: 600; margin-bottom: 0.5rem; color: #fff; }
    .meta { font-size: 0.85rem; color: #8a9bb0; margin-top: 0.75rem; }
    .stats-bar { display: flex; gap: 2rem; margin-top: 1.25rem; flex-wrap: wrap; }
    .stat { display: flex; flex-direction: column; }
    .stat-num { font-size: 1.5rem; font-weight: 700; color: #35DCC9; line-height: 1; }
    .stat-label { font-size: 0.75rem; color: #8a9bb0; margin-top: 0.2rem; }
    .country-bar { margin-top: 1.25rem; display: flex; flex-wrap: wrap; gap: 0.4rem; }
    ...
    .tier-icon { color: #35DCC9; font-size: 0.7rem; }
    .tier-count {
      margin-left: auto;
      background: #e8e8e0;
      color: #555;
    ...
    .job-title { font-size: 0.95rem; font-weight: 600; color: #0f1923; line-height: 1.3; }
    .company { font-size: 0.85rem; color: #555; margin-top: 0.15rem; }
    .location { font-size: 0.8rem; color: #777; margin-bottom: 0.6rem; }
    .apply-btn {
      white-space: nowrap;
      font-size: 0.8rem;
      font-weight: 500;
    ...
    .apply-btn:hover { background: #35DCC9; color: #0f1923; }
    ...
    .tag.country { background: #e8f4f2; color: #0f6e56; }
    .tag.salary { background: #fef3e2; color: #854f0b; }
    .tag.contract { background: #f0eefe; color: #534ab7; }
    .posted { font-size: 0.72rem; color: #aaa; }
    .no-roles {
      text-align: center;
      padding: 3rem 2rem;
      color: #777;
    ...
    footer a { color: #35DCC9; text-decoration: none; }
...
<header>
  <div class="header-inner">
    ...
    <p class="meta">Specialist roles posted in the last 14 days across 11 countries. Generated ${generatedAt}.</p>
    <div class="stats-bar">
      <div class="stat">
...
<main>
  ${jobs.length > 0 ? tierSections.trim() : noRoles}
</main>
<footer>
  <p>Curated for the security awareness and human risk community.</p>
  <p style="margin-top:0.4rem">Data sourced from Adzuna. Roles filtered to specialist awareness and human risk positions only. &nbsp;|&nbsp; <a href="https://antdavis.com">antdavis.com</a></p>
</footer>
</body>
</html>`;
}
...
async function main() {
  ...
  console.log(`\nKept ${allJobs.length} relevant roles after filtering.`);
